...
- 更大现金差额或任何外部渠道差异由最高权限账号处置；OWNER作为最终兜底可复核本人班次。
...
**AC-SHF-001 正常交班**  
Given 班次理论现金 1,000 元且没有待核对差异；When 收银员提交实交 1,000 元；Then 班次自动进入 `CLOSED`，金额、明细范围、操作者身份和时间形成不可变快照，不产生多余复核任务。
...
**AC-SHF-004 单OWNER异常班次兜底**
Given OWNER本人开班并提交了现金差额或外部待核对金额；When 该OWNER填写必填复核说明并确认复核；Then 允许班次进入 `CLOSED`，复核人与原操作者均记录为该OWNER，差额、说明和状态变化完整保留，流程不得因自我复核限制永久卡死。
