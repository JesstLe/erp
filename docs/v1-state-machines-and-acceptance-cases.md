...
V1 空库初始化阈值：店长必须同时满足“单行降幅不超过 10%”且“整单标准价与成交价差额绝对值不超过 50 元”才可自批；前台和收银员阈值为 0。高于标准价的手工改价默认也提交最高权限审批，优先通过新增真实服务项目表达增项。
...
**AC-FAC-010 可识别待录单**
Given `F01` 开始时选填会员 `M01` 和预计项目 `P01`，8 秒后结束；When 店长打开待录单队列；Then 主信息显示脱敏顾客、预计项目、设施、到店时间和“占用8秒”，机器接待号仅作追溯；只有明确点击“带入预计服务”才生成可编辑草稿行，且不自动保存、确认金额或收款。
...
**AC-ORD-002 店长阈值内自批**  
Given 标准总价 500 元，某行降幅 8%，整单差额 40 元且原因已填写；When `manager01` 确认价格；Then 审批为 `APPROVED`，保存完整价格快照和自批审计，订单可进入待支付。

**AC-ORD-003 任一阈值超限即升级**  
Given 单行降幅 9% 但整单差额 60 元，或单行降幅 12% 但整单差额 40 元；When 店长确认；Then 返回 `PRICE_APPROVAL_REQUIRED` 并创建 `PENDING_OWNER` 审批，未经批准不得开始结算。
...
**AC-AUD-001 敏感信息展示审计**  
Given 手机号默认脱敏；When 有权限用户执行“查看完整手机号”；Then 需要明确动作并记录操作者、顾客、原因和时间；无权限接口不返回明文。
