...
日期：2026-08-19
状态：真实字段画像完成；初步映射完成；尚未转换、导入或修改数据库
关联文档：[旧系统数据只读导出与迁移 PRD](legacy-system-data-migration-prd.md)

## 1. 本轮结论

本轮离线校验两批 AES-256-GCM 导出文件后，对 13 个模块、2544 条记录、213 个来源字段完成结构画像。画像报告只包含字段名、JSON 类型、出现/空值/去重数量、最大字符串长度、候选唯一键、格式类别和敏感级别，不包含任何来源字段值、值摘要、文件路径、账号、Cookie 或密钥。
...
| 模块 | 记录数 | 字段数 | 当前判断 |
|---|---:|---:|---|
| 顾客 `customers` | 2286 | 30 | 基础档案可转换；资金与权益字段阻断 |
...
| `customers.member_hand` | `T` | 清洗手机号后加密保存并生成查询 HMAC；2286 条非空但只有 2285 个不同值，至少一组重复候选必须人工复核 |
...
1. 缺少通用迁移运行、来源记录映射、字段转换版本和异常清单表。
