...
日期：2026-08-21
状态：65个结构化模块的真实字段画像与完整性校验已完成；护理文字记录保留、护理图片明确排除，B01受控重建待执行，资金权益仍需逐域对账
关联文档：[旧系统数据只读导出与迁移 PRD](legacy-system-data-migration-prd.md)

## 1. 本轮结论

本轮离线校验最新 AES-256-GCM 导出文件后，对65个模块、92920条记录、630个“模块内字段位”完成结构画像；其中29个模块存在记录。主体包括基础资料258条、顾客2311条、护理文字记录1760条、消费单31312条、消费明细49229条、充值单3458条、充值明细3425条和工资核算75条。画像报告只包含字段名、JSON类型、出现/空值/去重数量、最大字符串长度、候选唯一键、格式类别和敏感级别，不包含任何来源字段值、值摘要、文件路径、账号、Cookie或密钥。
...
| 模块 | 记录数 | 字段数 | 当前判断 |
|---|---:|---:|---|
| 顾客 `customers` | 2311 | 30 | 基础档案可转换；资金与权益字段需结合已导出消费/充值流水对账 |
| 护理记录 `care-records` | 1760 | 17 | 可转换成无收费历史服务档案；护理图片不在迁移范围 |
...
| 顾客消费单 `customer-sales` | 31312 | 38 | 作为订单/支付历史来源；需按状态和明细总额对账 |
| 消费服务/产品/次卡明细 | 49229 | 31-32 | 与消费单建立来源主从关系；孤儿明细阻断导入 |
| 顾客充值单及明细 | 6883 | 25-37 | 本金、赠送和支付拆分按来源字段验证，禁止只导总余额 |
| 工资核算 `payroll-data` | 75 | 30 | 仅作历史薪资资料；源元数据记录32、实际返回75行，必须单独对账 |

画像中的手机号、证件、余额、支付拆分、工资和自由文本均按敏感数据处理。三个基础字段包含HTML形态：`customers.member_code`、`products.goods_name`、`services.goods_name`。转换时只能使用受限解析器提取文本或标识，禁止把旧HTML原样写入或渲染到新系统。
...
| `customers.member_hand` | `T` | 清洗手机号后加密保存并生成查询HMAC；2311条记录只有2310个不同手机号，至少一组重复候选必须人工复核 |
...
| `care-records.bill_member` | `D` | 按旧顾客来源主键关联 `customers`；本次1760条在转换时必须逐条验证，缺失或未映射时记录异常并跳过该条 |
| `care-records.bill_shop` | `T` | 1760条覆盖5种旧门店值；按旧门店主键、代码或清洗后的名称唯一映射`store_id`，非空未命中时中止整批，禁止默认门店兜底 |
...
| 护理图片索引 | `A` | 当前产品范围明确不迁移护理图片；不读取、不下载、不进入归档或导入对账 |

### 4.12 历史消费、充值与发生门店

| 来源模块/字段 | 画像结果 | 新系统目标/处理 |
|---|---|---|
| `customer-sales.bill_shop` | 31312条全部非空，5种门店值 | 每张历史消费单的发生门店；按旧门店主键、编码或名称唯一映射到订单 `store_id` |
| `customer-topups.bill_shop` | 3458条全部非空，5种门店值 | 每张历史充值单的发生门店；用于充值流水和门店业绩归属，不能用顾客归属门店替代 |
| `customer-sales.bill_store` | 31312条全部非空，540种值 | 金额/储值候选字段，不是门店字段；须完成金额口径对账后再转换 |

顾客的 `home_store_id` 只表达建档/归属门店；消费、充值、护理必须分别保留单据自身的发生门店。同一顾客允许归属A店、在B店消费，这与品牌内跨店通用规则不冲突。
...
9. 当前已取得消费、充值、次卡和相关明细，但退款、积分、欠款等端点返回0条；必须区分“源站确实无记录”与“业务从未启用”，并完成金额及状态对账后才能建立期初权益。
...
| 编号 | 阻断项 | 解除条件 |
|---|---|---|
| MIG-B01 | 顾客资金字段语义仍需与已导出的消费/充值/支付明细对账 | 完成本金、赠送金、支付方式、退款/作废、积分和欠款的来源—目标逐域对账；0条模块需取得业务确认 |
...
1. 固化当前65模块基线，建立订单—明细—顾客—门店—员工的来源主从关系，并对0条业务域形成确认清单。
