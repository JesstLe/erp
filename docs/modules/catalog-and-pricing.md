...
已实现服务项目和产品基础目录，以及同时包含服务项目与产品的版本化标准价。产品包含编码、名称、计量单位和“后续是否管理库存”预留属性；该属性不会在当前版本生成库存余额、出入库或销售记录。价格草稿只允许 `OWNER` 创建和发布，发布后只读，服务订单按生效日期和发布时间选取服务项目标准价并保存快照。

产品销售、采购、库存流水、盘点、调拨、批次、效期、成本、目录编辑/停用和价格草稿修改仍为后续范围。操作说明见[产品目录与版本化标准价](../user-manual/10-product-catalog-and-pricing.md)。
