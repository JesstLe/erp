...
已实现服务项目和产品基础目录、产品可选封面图、版本化标准价、商品销售与门店整数库存。服务与商品不强制绑定，消费单可仅服务、仅商品或混合；价格草稿允许只提交本次变化项，未提交的已有价格继承为完整快照。价格草稿只允许 `OWNER` 创建和发布，发布后只读，消费单按生效日期和发布时间选取价格版本并保存服务/商品价格快照。

采购、盘点单、调拨、批次、效期、成本、目录编辑/停用和价格草稿修改仍为后续范围。操作说明见[产品目录与版本化标准价](../user-manual/10-product-catalog-and-pricing.md)和[商品销售与门店库存](../user-manual/15-product-sales-and-inventory.md)。
...
- 服务项目与产品是独立目录，不要求一一绑定；价格版本可只调整任一类型，未调整的已有价格继承为该版本不可变快照。
- 从未定价的目录条目不因其他条目发布而自动获得零价，也不阻塞已选条目发布；只有明确发布价格后才能进入新消费单。
- 门店店长不能修改价格档案；结算时只能在授权范围内确认实际成交价，并记录参考价格版本、差额和原因。
- 店长发生改价必须填写原因；系统同时支持金额差和比例差阈值，任一超过配置时提交最高权限账号复核，不能先结算后补审批。
- V1空库初始化值为：单行降价比例不超过10%且各优惠行汇总不超过50元时，店长可以在填写原因后直接执行；优惠金额不与其他行提价抵消，任一超限或手工高于标准价均由最高权限账号审批。策略使用只增不改的版本配置，订单与审批保存当时快照，禁止把阈值写死在订单逻辑中。
- 停用主数据只阻止新业务引用，不影响历史单据、退款、报表和库存追溯。
- 服务项目的标准时长用于排班和经营分析，不自动换算收费；设施使用时长也不改变项目价格。
- 产品必须明确是否管理库存、批次、效期和成本；非库存服务项目不能产生库存流水。
- 产品封面图可选，在产品目录与收银商品选择列表中辅助辨认；上传、更换或缺失图片都不得改变价格版本、库存或历史业务快照。
