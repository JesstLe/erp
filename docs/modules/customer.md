...
旧版页面采用“顶部工具栏 + 左侧顾客卡类树 + 右侧宽表格”的操作结构。工具栏顺序固定为：新增、修改、批量修改、删除、查询、导入、刷新、表格、打印、导出、退出。列表字段依次包含会员卡号、会员姓名、性别、手机号码、办卡分店、会员卡类、来店渠道、储值余额、储值奖励、消费总额、欠款金额、签单额度、最后来店时间、登记时间、更新时间、会员状态和备注。

查询弹窗包含办卡门店、会员类别、会员状态、生日月份、推荐顾客和顾客查询；“推荐顾客”和“顾客查询”均支持按卡号或完整手机号定位。新版经典界面保持该信息架构，但输入关键词后自动检索，不再要求二次点击搜索。

新增档案表单已确认包含：照片、会员卡类、会员卡号、手机号、姓名、性别、生日、会员密码、办卡分店、折扣方式、产品/服务/次卡折扣、签单额度、推荐顾客、来店渠道、状态、有效期、邮箱、消费短信、营销短信、提成折扣和备注。新系统把“创建顾客档案”和“开通会员/建立资金账户”拆开，避免普通顾客被强制开卡或储值。

### 经典版当前接入状态

- `/ui/new/customer` 已按旧版改为双图表、最新登记顾客列表和右侧两组快捷入口；使用真实顾客摘要与卡类目录，不伪造迁移数据。
- `/ui/new/customer/list` 已接入真实顾客查询、自动索引、选中行、新增、修改、逻辑停用/恢复、刷新、表格密度、打印、脱敏导出和退出。
- `/ui/new/legacy/customer/customer-005` 是独立的经典版服务档案页面，不嵌入或跳转到现代版顾客页面。它直接读取当前门店服务档案，支持按顾客姓名、完整手机号或手机号尾号自动检索，并保留经典版“顶部工具栏 + 左侧分类树 + 右侧宽表格”的操作结构。
- “顾客护理分类”已改为品牌级可配置的“服务记录分类”：分类名称、顺序和启停状态由品牌维护，唯一编码由系统自动生成且不随改名变化。系统不再把灰指甲、手足癣、技师等参考行业词作为所有品牌的固定字段。
- 新增服务档案时分类可选，并可从档案页直接维护分类；历史记录和未选择分类的记录显示为“未分类”。档案可记录护理老师和下次回访时间，回访到期前进入右上角护理提醒。补充更正不要求员工填写原因，系统仍保留原文、操作人和时间。
- “删除”按新系统数据安全规则映射为逻辑停用，保留历史订单、余额、服务记录和审计；这项行为与旧版物理删除按钮文案不同，但不会破坏账务链路。
- 批量修改、顾客导入、按具体卡类筛选、卡类数量聚合和储值日趋势目前缺少后端接口，界面保留旧版位置并明确提示，不使用假数据冒充已完成能力。
- 开卡、储值、预约、护理、退款、积分与兑换等页面继续使用已采集的旧版字段清单呈现；后续逐页连接现有会员、支付、预约、服务记录、退款和库存接口。
...
- 护理/服务档案：记录服务过程和顾客注意事项，敏感信息单独授权；服务记录分类属于品牌数据字典，不属于页面写死枚举。
...
- 顾客主档属于品牌租户，不属于某一家门店；建档门店只作为来源信息。A 店建立的顾客和会员，B 店在具备顾客/会员权限时可以查询、识别和消费。
- 手机号、会员卡号、储值本金、奖励金、积分和默认通用次卡在品牌内统一；不同品牌即使手机号相同，也形成彼此隔离的顾客、卡和账户。
- 订单、支付、储值、权益核销和服务记录保留实际发生门店，以支持门店业绩、交班、退款、跨店核算与审计；共享会员权益不等于把各门店经营流水合并成一笔。
...
- 服务档案与普通消费备注分离，按敏感数据处理；当前只允许最高权限账号和店长创建、查看，历史档案不允许覆盖或物理删除。
- 服务记录分类在品牌内共享、跨门店可复用，不同品牌之间按 `tenant_id` 完全隔离。分类已被历史档案引用时不能物理删除，但可以停用；停用不影响历史记录显示。
- 服务档案可选关联同一顾客的消费单，但不重复保存消费金额；服务时间由系统默认，情况/需求、服务过程、结果建议和最多6张图片均可选填。
- 服务图片加密存放在发布目录之外，每次读取重新校验租户、门店、顾客和角色；数据库只保存文件元数据与完整性摘要。
...
| 顾客护理 | 品牌自定义服务记录分类、服务档案列表、新增和追加更正 | 分类名称不得绑定特定行业；记录可能包含敏感信息，必须单独授权、留痕且不覆盖原文 |
...
- 查询列表按原名展示顾客姓名，手机号统一显示为前三位加四个星号加后四位；输入完整手机号时使用检索哈希精确匹配，列表接口不返回手机号明文。
