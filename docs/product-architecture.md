...
当前默认边界：桌面端 Web 为主；一个平台服务多个相互隔离的品牌租户，每个品牌租户内部支持总部、区域和多门店。品牌内共享顾客身份与会员权益，门店记录业务发生地；品牌之间业务数据完全隔离。平台控制面独立处理商户注册审核、商户启停和跨商户登录安全事件，但默认不直接进入商户业务数据；保留未来一个经营主体管理多个品牌和移动端扩展能力。新系统从空库开始，不迁移旧系统历史业务数据。当前2核4G、70GB服务器迁移为Ubuntu Server 24.04 LTS，作为测试和第一版生产基线。数据库采用PostgreSQL 18，结构变更采用Flyway版本化迁移；详细约束见 [品牌租户与多门店数据边界](modules/brand-multistore-data-boundary.md)、[数据库设计与变更治理规范](database-design-and-change-governance.md)、[Linux单机部署与容量基线](linux-server-deployment-baseline.md) 和 [技术架构与开发基线](technical-architecture-and-stack.md)。
