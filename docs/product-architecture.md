...
关键原则：“打开设施”是查看，“开始使用”才创建接待与设施使用记录；开始时允许顾客身份未知，结算前再关联会员。设施在服务结束时释放，不等待支付完成。设施计时只记录运营信息，不参与自动计价；最终收费金额由店长输入或确认。设施使用、服务内容确认、权益核销和支付分别产生独立业务事件。详细设计见 [门店接待、设施计时与收银领域设计](domains/store-service-and-cashier.md)。
