...
当前默认边界：桌面端 Web 为主；一个平台服务多个相互隔离的品牌租户，每个品牌租户内部支持总部、区域和多门店。品牌内共享顾客身份与会员权益，门店记录业务发生地；品牌之间业务数据完全隔离。平台控制面独立处理商户注册审核、商户启停和跨商户登录安全事件，但默认不直接进入商户业务数据；保留未来一个经营主体管理多个品牌和移动端扩展能力。新系统数据库从空库建立，在正式启用前通过独立只读迁移工具导出、对账并受控导入旧系统历史数据，迁移程序不得修改旧系统。当前2核4G、70GB服务器迁移为Ubuntu Server 24.04 LTS，作为测试和第一版生产基线。数据库采用PostgreSQL 18，结构变更采用Flyway版本化迁移；详细约束见 [旧系统数据只读导出与迁移 PRD](legacy-system-data-migration-prd.md)、[品牌租户与多门店数据边界](modules/brand-multistore-data-boundary.md)、[数据库设计与变更治理规范](database-design-and-change-governance.md)、[Linux单机部署与容量基线](linux-server-deployment-baseline.md) 和 [技术架构与开发基线](technical-architecture-and-stack.md)。
...
- 支付、短信、打印及第三方账户
- 操作日志、登录日志和敏感数据访问日志
- 平台控制面独立于商户 `OWNER`：平台管理员、商户注册审核、商户启停和跨商户登录安全事件采用独立认证与审计边界
- 开放接口、Webhook和同步任务
...
关键原则：“打开设施”是查看，“开始使用”才创建接待与设施使用记录；开始时允许顾客身份未知，也可选填顾客与预计服务来帮助后续识别，结算前仍可关联会员。设施在服务结束时释放，不等待支付完成。设施计时和预计服务只记录运营信息，不参与自动计价；最终收费金额由店长输入或确认。待录单界面必须优先展示顾客原名、预计服务、设施和时间，机器编号仅用于追溯。设施使用、服务内容确认、权益核销和支付分别产生独立业务事件。详细设计见 [门店接待、设施计时与收银领域设计](domains/store-service-and-cashier.md)。
...
| D2 | 部署与租户 | 平台支持多个品牌租户及商户管理后台；品牌内顾客/会员共享，品牌间按 `tenant_id` 隔离 | 同时满足连锁跨店经营与多商户安全边界 |
...
| D8 | 当前部署 | Ubuntu Server 24.04 LTS单机承载测试和第一版生产应用与数据库；代码保持跨平台 | 决定当前资源限制，但不形成长期生产绑定 |
| D9 | 外部支付 | 形成微信 Native 与支付宝订单码开发设计；正式接入前仍使用人工登记待核对 | 决定支付中心、回调安全、退款和渠道对账边界 |
| D10 | 第一版技术架构 | .NET 10模块化单体 + React 19 + PostgreSQL 18 + Flyway；V1不上Kubernetes | 支持当前低资源Linux环境并保留后续扩容能力 |
| D11 | 历史数据 | 新系统结构从空库建立；正式启用前通过只读接口导出旧系统数据，经加密留存、字段映射和数量/资金对账后受控导入 | 保留历史经营连续性，同时以端点白名单和独立迁移任务保证旧系统零写入 |
