...
日期：2026-08-20
状态：顾客、基础资料、护理列表与顾客照片只读导出已实现；B01 测试品牌生产演练已完成
...
2026-08-20 起，迁移工具增加服务器管理员专用导入命令，当前代码只允许目标品牌 `B01`。默认完整执行转换和约束检查后回滚；只有显式 `--apply` 才提交。该切片导入门店、员工档案、服务、产品、卡类、顾客基础档案以及由顾客备注/照片形成的历史服务档案。
...
最终两批合计完成 13 个结构化模块、2545 条原始记录的只读加密导出；另完成护理列表0条和顾客档案照片139张。导出完成不等于迁移完成，B01 仍须依次通过备份、干跑、差异核对、显式提交和幂等复跑，详见 9.3 和 6.1。

### 9.3 顾客增量、护理列表与档案照片

2026-08-20 复查时旧系统顾客从 2286 条增加到 2287 条，因此重新执行 23 页全量导出，不能沿用过期人数作为最终迁移基线。

“顾客护理”页面及其 `GET /swshop/vip/nurse.php?act=grid` 已登记。源系统在零记录时返回空响应而不是 jqGrid JSON；工具只对这个精确端点把空响应规范化为 0 条，其余端点空响应仍然停止。当前源护理列表为 0 条。
...
顾客自由文本 `member_memo` 已在顾客列表导出中；正式导入时只将非空备注或存在照片的档案转换成带“旧系统顾客档案备注（迁移）”标记的服务档案，不伪造消费单和收费金额。

2026-08-20 最终运行已逐份检查2287个顾客档案，成功导出139张照片、2148份档案无照片；照片清单数与实际密文文件数均为139，密文 SHA-256 不一致为0。
...
| 数据域 | 页面入口 | 已确认读取端点 | 状态 |
|---|---|---|---|
| 顾客 | `base/frame.php?act=member` | `GET /swshop/base/member.php?act=grid` | 已完成，2287 条 |
