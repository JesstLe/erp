...
旧顾客当前 `member_money` 作为可消费本金，`member_bonus + member_sbonus` 作为可消费赠送金；累计储值、信用、欠款和积分只进入 `legacy_customer_financial_snapshots` 非消费证据。期初余额以不可变迁移流水入账，不生成充值订单或支付，因此不计入迁移当日收入。
