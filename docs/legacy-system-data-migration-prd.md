...
日期：2026-08-21
状态：顾客、基础资料和1743条护理记录已只读导出；护理图片全量导出于2026-08-22按产品决定暂缓，B01 测试品牌等待按修正映射重建
...
2026-08-20 起，迁移工具增加服务器管理员专用导入命令，当前代码只允许目标品牌 `B01`。默认完整执行转换和约束检查后回滚；只有显式 `--apply` 才提交。该切片导入门店、员工档案、服务、产品、卡类、顾客基础档案，以及由顾客备注/照片和护理记录/护理照片分别形成的历史服务档案。

旧员工不自动创建账号；旧服务、产品和卡类使用明确的 `LEGACY-*` 技术编码；旧门店由新系统业务编码生成器分配不可变 `S###` 编号。每条目标记录写入来源实体、来源主键、来源 SHA-256 和目标主键映射，内容变化时拒绝静默覆盖。

旧顾客余额、赠送、累计储值、信用、欠款和积分不写入任何可消费账户，只进入 `legacy_customer_financial_snapshots`。该表以数据库检查约束固定 `is_spendable=false`，同时与来源顾客和目标顾客关联，用于授权对账，不参与收银。
...
- B01历史门店 `S01` 保持不变，新导入门店取得 `S002` 至 `S006`；另一品牌全部业务与迁移计数保持原基线。

2026-08-21 的业务验收发现首轮导入存在门店归属缺陷：旧顾客和员工的门店字段是名称文本，而导入器只匹配旧主键和门店代码，未命中时又静默使用第一家旧门店。因此首轮导入的顾客、员工和服务档案门店归属不能作为正式迁移结果。整改后的导入器同时登记旧主键、门店代码和门店名称；空值才允许使用明确的演练默认门店，任何非空未映射值或重复名称均中止整个事务。B01 必须从可验证备份重建或按受控纠正任务重放，禁止在错误归属上继续猜测更新。
...
2026-08-22 产品决定：护理图片文件数量大、旧站响应慢，本阶段暂停继续追求全量下载，把开发和验收资源转回新 ERP 核心业务闭环。该决定只暂停附件下载，不把当前检查点改写成“全量完成”：1743条护理结构化记录仍是已验证基线，已下载且通过摘要校验的图片可以保留，未遍历或未下载部分必须继续标记为待补迁。恢复任务时必须沿用现有检查点和失败清单续跑，不得从头反复请求旧站；只有详情遍历完成、失败清单为0、清单摘要一致并与导入附件逐项对账后，文档才允许标记“护理图片全量完成”。护理图片未完成不阻塞使用合成数据完成新系统业务流程测试，但阻塞旧系统附件迁移的正式验收。
...
| 数据域 | 页面入口 | 已确认读取端点 | 状态 |
|---|---|---|---|
| 顾客 | `base/frame.php?act=member` | `GET /swshop/base/member.php?act=grid` | 已完成，2293条 |
| 护理记录 | `vip/nurse.php` | `GET /swshop/vip/nurse.php?act=grid`（含完整查询参数） | 结构化记录已完成1743条；护理图片于2026-08-22暂缓，保留检查点待补迁，不视为全量完成 |
