...
- Linux服务器使用无登录权限的独立应用账号，密钥文件由 `root:erp` 管理并以最小读取权限提供。
...
当前环境使用Ubuntu Server 24.04 LTS，支付开发阶段至少需要：

- 独立测试域名和有效 HTTPS 证书；渠道通知地址必须能从公网访问。
- Nginx只向应用转发必要入口，不暴露数据库、Kestrel回环端口和管理端口。
- PostgreSQL、应用、密钥目录、日志和账单文件使用不同账号或最小权限Unix所有者/组/模式。
