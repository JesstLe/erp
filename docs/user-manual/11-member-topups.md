...
页面没有“直接改余额”或“删除流水”入口。输入错误时从储值记录申请整单冲正；规则和审批步骤见[消费退款与会员储值整单冲正](13-payment-refunds-and-topup-reversals.md)。
...
当前已开放现金储值的整单冲正：必须退回全部实收本金并收回全部赠送奖励，且由 `OWNER` 批准。任一部分已经消费时拒绝冲正，不会修改原充值流水或只冲一半。
