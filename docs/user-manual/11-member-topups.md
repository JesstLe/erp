...
| 字段 | 必填 | 规则 |
|---|---|---|
| 储值本金 | 是 | 打开窗口默认 0，录入实际收款且必须大于 0；以整数分发送到服务端 |
| 赠送奖励金 | 是 | 默认 0；只有最高权限账号可填写正数 |
| 支付方式 | 是 | 不能选择会员账户给自身充值；同一方式不可重复 |
| 实收金额 | 是 | 打开窗口默认 0；所有支付分摊合计必须等于储值本金，不包含奖励金 |
| 交易参考号 | 条件必填 | 微信人工登记可留空；支付宝及其他人工外部收款为 4–100 字 |
| 储值备注 | 否 | 最多 500 字 |

点击“确认收款并入账”后，储值单、支付单、支付分摊、本金/奖励金账户流水、余额投影、审计和幂等结果在同一个数据库事务内完成。任一环节失败时整体回滚。

储值成功后弹窗保留本次入账结果，可立即点击“打印储值小票”；小票包含门店、储值单号、支付单号、会员、卡号、本金、赠金和支付方式。

## 4. 查看余额与储值记录

顾客详情上方将“会员储值”和“服务档案”作为同级页签，店长可以在同一位置快速切换资金记录与服务过程。
