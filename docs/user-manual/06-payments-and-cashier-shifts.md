...
- 会员储值扣款、真实渠道二维码/回调、退款和渠道账单对账尚未开放。
...
| 现金 | 已记录 | 无需外部渠道对账 |
| 微信/支付宝人工登记 | 人工登记 | 待核对 |
| 真实微信/支付宝（未开放） | 只有合法回调或主动查单才能渠道确认 | 等待渠道账单匹配 |
| 会员内部账户（未开放） | 系统内部扣减成功 | 无需外部渠道对账 |
...
- 存在差额或待核对金额时，复核说明必填。
- 当前本地种子环境只有一个 `OWNER` 账号，无法违反职责分离进行自我复核；需要创建第二个授权账号后才能完成关班验收。
...
- 现金找零、签单欠款、会员储值/奖励金组合扣款和 500 元验证码规则。
- 新增员工账号、班次接续、交班退回修改和超过 24 小时未复核告警。
