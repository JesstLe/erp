...
- “交班复核”：异常待复核和已关闭班次；账实一致且没有外部待核对的班次会在提交时自动关闭，服务端继续强制禁止异常班次由原收银员自我复核。
...
- 存在任意现金差额或外部待核对金额：状态进入“待复核”，必须由另一名有权限人员处理；原收银员本人不能复核自己的异常班次。
...
- 异常交班复核只允许 `OWNER` 或门店店长，并且复核人不能是本班次收银员；正常无差额班次由系统自动关班。
