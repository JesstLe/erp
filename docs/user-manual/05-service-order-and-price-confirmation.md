...
在“设施接待”中打开服务位时，添加项目或产品后即可直接在左侧清单修改“本次成交价”。页面同时保留目录价作对照，并实时重算行小计与整单合计；成交价不同于目录价时自动显示改价标记和改价原因输入框。默认原因是“现场调整成交价”，操作人可以按实际情况改写。该改价只作用于本次消费单，不会修改服务项目、产品目录或已发布价格版本。

设施接待加载当前品牌全部“启用”状态的服务项目和产品，不再因为最新价格版本缺少某条价格而把主档隐藏。已发布价格行正常显示目录价；没有价格行的项目显示“未设置目录价”，加入清单后必须先填写大于零的本次成交价才能保存或结算。服务端把这类金额按“标准价为零的本次改价”处理，继续执行角色权限、审批与审计规则。若要让门店日常使用标准价而不触发改价审批，应由最高权限账号把相应项目加入新的已发布价格版本。
