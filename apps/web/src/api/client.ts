...
let platformCsrfToken: string | undefined

export function resetCsrfToken(): void {
  csrfToken = undefined
  platformCsrfToken = undefined
}

async function getCsrfToken(platform: boolean): Promise<string> {
  if (platform && platformCsrfToken) return platformCsrfToken
  if (!platform && csrfToken) return csrfToken
  const response = await fetch(platform ? '/api/v1/platform/auth/csrf' : '/api/v1/security/csrf',
    { credentials: 'include' })
  if (!response.ok) throw new ApiError(response.status, await safeJson(response))
  const body = (await response.json()) as { token: string }
  if (platform) platformCsrfToken = body.token
  else csrfToken = body.token
  return body.token
}
...
export async function apiRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const platform = path.startsWith('/api/v1/platform/') && path !== '/api/v1/platform/auth/login'
  const headers = new Headers(init.headers)
  headers.set('Accept', 'application/json')
  if (init.body && !(init.body instanceof FormData)) headers.set('Content-Type', 'application/json')
  if (unsafe) headers.set('X-CSRF-TOKEN', await getCsrfToken(platform))
  const response = await fetch(path, { ...init, headers, credentials: 'include' })
  if (!response.ok) {
    const payload = await safeJson(response)
    if (unsafe && payload.error?.code === 'INVALID_ANTIFORGERY_TOKEN') {
      if (platform) platformCsrfToken = undefined
      else csrfToken = undefined
      headers.set('X-CSRF-TOKEN', await getCsrfToken(platform))
