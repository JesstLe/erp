    ...
    <Layout>
      <Header className="app-header">
        ...
        <div className="header-actions">
          <Popover trigger={['hover', 'click']} mouseEnterDelay={0.12} placement="bottomRight" onOpenChange={(open) => { if (open) void notifications.refetch() }} content={<div className="header-popover notification-popover"><div className="header-popover-title"><strong>待办通知</strong><Typography.Text type="secondary">{notifications.data?.pendingCount ?? 0} 条待处理</Typography.Text></div>{notifications.data?.items.length ? <div className="notification-list">{notifications.data.items.map((item) => <button type="button" key={item.id} className="notification-item" onClick={() => navigate(item.targetUrl)}><span><strong>{item.title}</strong><Tag color={item.severity === 'error' ? 'red' : item.severity === 'warning' ? 'gold' : 'blue'}>{item.type === 'PriceOverrideApproval' ? '改价' : item.type === 'RefundApproval' ? '退款' : item.type === 'CareFollowUp' ? '护理' : '交班'}</Tag></span><small>{item.description}</small><time>{new Date(item.occurredAtUtc).toLocaleString('zh-CN', { hour12: false })}</time></button>)}</div> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={notifications.isLoading ? '正在加载' : '暂无待办'} />}</div>}>
            <Badge count={notifications.data?.pendingCount ?? 0} overflowCount={99} size="small"><Button type="text" className="header-icon-button" icon={<BellOutlined />} aria-label="待办通知" title="待办通知" /></Badge>
          </Popover>
