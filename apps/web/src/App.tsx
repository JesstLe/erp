...
export default function App() {
  return <Routes><Route path="/login" element={<LoginPage />} /><Route element={<ProtectedRoute />}><Route element={<AppLayout />}><Route index element={<DashboardPage />} /><Route path="catalog/items" element={<ServiceItemsPage />} /><Route path="catalog/prices" element={<PriceBooksPage />} /><Route path="facilities" element={<ComingSoonPage title="设施接待" />} /><Route path="customers" element={<ComingSoonPage title="顾客与会员" />} /><Route path="cashier" element={<ComingSoonPage title="服务录单与收银" />} /><Route path="reports" element={<ComingSoonPage title="经营报表" />} /><Route path="audit" element={<ComingSoonPage title="审计记录" />} /></Route></Route><Route path="*" element={<Navigate to="/" replace />} /></Routes>
}
