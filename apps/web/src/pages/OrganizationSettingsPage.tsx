...
export function OrganizationSettingsPage() {
  ...
  const saveStore = useMutation({ mutationFn: (values: StoreValues) => editingStore === 'new' ? apiRequest<StoreProfile>('/api/v1/organization/stores', { method: 'POST', body: JSON.stringify(values) }) : apiRequest<StoreProfile>(`/api/v1/organization/stores/${editingStore!.id}`, { method: 'PUT', body: JSON.stringify({ ...values, expectedVersion: editingStore!.version }) }), onSuccess: async () => { const created = editingStore === 'new'; setEditingStore(undefined); storeForm.resetFields(); await refresh(created ? '门店已创建，可在员工与设施配置中继续初始化' : '门店资料已更新', true) }, onError })
  ...
  const openStore = (store: StoreProfile | 'new') => { storeForm.setFieldsValue(store === 'new' ? { code: '', name: '', timeZoneId: 'Asia/Shanghai' } : { code: store.code, name: store.name, timeZoneId: store.timeZoneId }); setEditingStore(store) }
  ...
  return <div className="page-stack">
    ...
    <Modal title="编辑品牌资料" open={brandOpen} onCancel={() => setBrandOpen(false)} onOk={() => brandForm.submit()} okText="保存" confirmLoading={updateBrand.isPending} destroyOnHidden><Form<BrandValues> form={brandForm} layout="vertical" onFinish={(values) => updateBrand.mutate(values)}><Form.Item name="code" label="品牌编码" rules={[{ required: true }, { max: 32 }, { pattern: /^[A-Za-z0-9_-]+$/, message: '仅限字母、数字、下划线和短横线' }]}><Input maxLength={32} /></Form.Item><Form.Item name="name" label="品牌名称" rules={[{ required: true }, { max: 100 }]}><Input maxLength={100} /></Form.Item></Form></Modal>
    <Modal title={editingStore === 'new' ? '新增门店' : '编辑门店'} open={Boolean(editingStore)} onCancel={() => setEditingStore(undefined)} onOk={() => storeForm.submit()} okText="保存" confirmLoading={saveStore.isPending} destroyOnHidden><Form<StoreValues> form={storeForm} layout="vertical" onFinish={(values) => saveStore.mutate(values)}><Form.Item name="code" label="门店编码" rules={[{ required: true }, { max: 32 }, { pattern: /^[A-Za-z0-9_-]+$/, message: '仅限字母、数字、下划线和短横线' }]}><Input maxLength={32} placeholder="例如 S02" /></Form.Item><Form.Item name="name" label="门店名称" rules={[{ required: true }, { max: 100 }]}><Input maxLength={100} /></Form.Item><Form.Item name="timeZoneId" label="业务时区" rules={[{ required: true }, { max: 64 }]}><Input maxLength={64} placeholder="Asia/Shanghai" /></Form.Item></Form></Modal>
