...
export function ModernFacilityCashierWorkbench({ facility, availableFacilities, onFacilityChanged, onExit, onCompleted }: Props) {
  ...
  const openSettlement = () => { if (isBeforeStart) { message.warning('请先确认录单信息并点击“开始计时”'); return } if (!lines.length) { message.warning('请先选择项目或产品'); return } if (lines.some((line) => line.lineType === 'Service' && !line.employeeId)) { message.warning('请先通过“整单员工”或明细选择服务员工'); return } const method = paymentMethods.data?.find((item) => !item.channelProvider && item.code !== 'GROUP_BUY_MANUAL') ?? paymentMethods.data?.[0]; settleForm.resetFields(); settleForm.setFieldsValue({ methodId: method?.id, amountYuan: totalMinor / 100, cashTenderedYuan: totalMinor / 100, groupBuyPlatform: '美团' }); setSettleOpen(true) }
    ...
    <div className="modern-cashier-body">
      <section className="modern-bill-pane">
        ...
        {!lines.length ? <Empty description="当前没有选择消费的任何项目或产品" /> : <div className="modern-bill-lines">{lines.map((line, index) => <article key={line.key}>
          ...
          <div className="modern-line-meta"><span>编号 {line.code}</span><span>目录价 {money(line.referencePriceMinor)}</span>{line.enteredPriceMinor !== line.referencePriceMinor && <Tag color="orange">已改价</Tag>}<strong>小计 {money(classicCashierLineAmount(line))}</strong></div>
