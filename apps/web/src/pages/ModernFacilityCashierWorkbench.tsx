...
function applyMembershipPricing(lines: ClassicCashierDraftLine[], customer?: CustomerDetail) {
  const today = new Date().toISOString().slice(0, 10)
  const activeCards = customer?.cards.filter((card) => card.status.toUpperCase() === 'ACTIVE' &&
    card.validFrom <= today && (!card.validTo || card.validTo >= today)) ?? []
...
function fromOrder(order: ServiceOrder): ClassicCashierDraftLine[] {
  return order.lines.map((line) => ({
    ...
    memberCardTypeId: line.memberCardTypeId,
    memberCardTypeName: line.memberCardTypeName,
  }))
}
...
export function ModernFacilityCashierWorkbench({ facility, availableFacilities, onFacilityChanged, onExit, onCompleted }: Props) {
  ...
  const previewCustomerDetail = useQuery({ queryKey: ['customer-detail', storeId, previewCustomerId], enabled: Boolean(storeId && previewCustomerId), queryFn: () => apiRequest<CustomerDetail>(`/api/v1/customers/${previewCustomerId}?storeId=${storeId}`) })
  ...
  const memberSearchPending = hasMemberSearch && memberSearch.trim() !== debouncedMemberSearch
  ...
  const addService = async (item: typeof serviceCatalog[number]) => persistLines(applyMembershipPricing([...linesRef.current, { key: commandId(), lineType: 'Service', itemId: item.id, code: item.code, name: item.name, quantity: 1, actualMinutes: item.duration, referencePriceMinor: item.priceMinor, referencePriceDefined: item.hasPublishedPrice, enteredPriceMinor: item.priceMinor, pricingSource: 'ListPrice' }], customerDetail.data))
  ...
  const confirmAddProduct = async () => {
    ...
    await persistLines(applyMembershipPricing([...linesRef.current, { key: commandId(), lineType: 'Product', itemId: item.id, code: item.code, name: item.name, unitName: item.unitName, quantity: 1, referencePriceMinor: item.priceMinor, referencePriceDefined: item.hasPublishedPrice, enteredPriceMinor: item.priceMinor, employeeId: employee?.id, employeeName: employee?.displayName, pricingSource: 'ListPrice' }], customerDetail.data))
  }
  ...
  const updateLinePrice = (line: ClassicCashierDraftLine, yuan: number | null) => {
    ...
    updateLine(line.key, {
      ...
      priceOverrideReason: enteredPriceMinor === line.referencePriceMinor
        ? undefined
        : line.pricingSource === 'MemberDiscount' ? '现场调整成交价' :
          line.priceOverrideReason?.trim() || '现场调整成交价',
      pricingSource: enteredPriceMinor === line.referencePriceMinor ? 'ListPrice' : 'ManualOverride',
  ...
  const startReception = useMutation({
    ...
    onSuccess: async ({ started, saved }) => { setLocalLines(fromOrder(saved)); onFacilityChanged(started); setServiceEnded(false); await onCompleted(); message.success('接待信息已保存，设施现在开始计时') },
    onError: (error) => message.error(requestError(error)),
  })
  ...
  const settleMutation = useMutation({
    mutationFn: async (values: SettlementValues) => {
      ...
      setLocalLines(fromOrder(order)); setCustomerId(overriddenCustomerId); setNote(overriddenNote)
    ...
    <div className="modern-cashier-body">
      <section className="modern-bill-pane">
        ...
        {!lines.length ? <Empty description="当前没有选择消费的任何项目或产品" /> : <div className="modern-bill-lines">{lines.map((line, index) => <article key={line.key}>
          ...
          <div className="modern-line-meta"><span>编号 {line.code}</span><span>{line.referencePriceDefined === false ? '未设置目录价' : `目录价 ${money(line.referencePriceMinor)}`}</span>{line.referencePriceDefined === false && line.enteredPriceMinor <= 0 ? <Tag color="red">请填写成交价</Tag> : line.pricingSource === 'MemberDiscount' ? <Tag color="green">{(Number(line.memberDiscountBasisPoints ?? 10_000) / 1000).toFixed(3).replace(/0+$/, '').replace(/\.$/, '')}折会员价 · 优惠 {money(Math.max(0, line.referencePriceMinor - line.enteredPriceMinor))}</Tag> : line.pricingSource === 'ManualOverride' && <Tag color="orange">人工改价</Tag>}<strong>小计 {money(classicCashierLineAmount(line))}</strong></div>
      ...
      <section className="modern-catalog-pane">
        ...
        {tab === 'member' && <div className="modern-member-search">
          ...
          <div className="modern-member-results">{!hasMemberSearch && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="请输入姓名、完整手机号或卡号后查询会员" />}
            {(memberSearchPending || customers.isFetching) && <Spin size="small" />}
            {hasMemberSearch && !memberSearchPending && customers.data?.map((customer) => <button type="button" key={customer.id}
            ...
            {hasMemberSearch && !memberSearchPending && !customers.isFetching && customers.data?.length === 0 && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="未找到匹配会员" />}
          </div>
