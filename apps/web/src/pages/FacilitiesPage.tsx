...
interface StartValues { customerId?: string; plannedServiceItemId?: string; expectedDurationMinutes?: number; note?: string }
...
export function FacilitiesPage() {
  ...
  const [workbenchFacility, setWorkbenchFacility] = useState<FacilityBoardItem>()
  ...
  const start = async (values: StartValues) => { if (!startTarget || !storeId) return; const started = await mutate.mutateAsync({ path: '/api/v1/facilities/sessions/start', body: { storeId, facilityId: startTarget.id, ...values, commandId: commandId() } }); message.success('设施已开始使用'); setStartTarget(undefined); startForm.resetFields(); if (canCheckout) setWorkbenchFacility(started) }
  ...
  const { can } = useAuthorization(); const canConfigure = can(Permission.FacilityConfigure); const canCheckout = can(Permission.CashierCheckout)
  const openFacility = async (item: FacilityBoardItem) => {
    if (!canCheckout) { if (item.status === 'AVAILABLE') setStartTarget(item); return }
    if (item.sessionId && ['IN_USE', 'PAUSED'].includes(item.status)) { setWorkbenchFacility(item); return }
    if (item.status !== 'AVAILABLE' || !storeId || mutate.isPending) return
    const started = await mutate.mutateAsync({ path: '/api/v1/facilities/sessions/start', body: { storeId, facilityId: item.id, commandId: commandId() } })
    setWorkbenchFacility(started); message.success('已开始接待，请选择本次消费项目或产品')
  }

  if (workbenchFacility) return <ModernFacilityCashierWorkbench
    facility={workbenchFacility}
    availableFacilities={available.filter((item) => item.id !== workbenchFacility.id)}
    onFacilityChanged={setWorkbenchFacility}
    onCompleted={refresh}
    onExit={() => { setWorkbenchFacility(undefined); void refresh() }}
  />
  ...
  return <div className="page-stack">
    ...
    {board.data?.groups.map((group) => <section key={group.id} className="facility-group"><Typography.Title level={4}>{group.displayName}<Typography.Text type="secondary"> · {group.facilities.length} 个设施</Typography.Text></Typography.Title><div className="facility-grid">{group.facilities.map((item) => {
      ...
      const canOpenWorkbench = canCheckout && (item.status === 'AVAILABLE' || Boolean(item.sessionId && ['IN_USE', 'PAUSED'].includes(item.status)))
      return <Card key={item.id} className={`facility-card status-${item.status.toLowerCase()}${canOpenWorkbench ? ' is-actionable' : ''}`} hoverable={canOpenWorkbench || item.status === 'AVAILABLE'} onClick={() => void openFacility(item)}>
        ...
        {item.sessionId ? <><Statistic title="实际占用时长" value={formatDuration(liveSeconds)} prefix={<ClockCircleOutlined />} /><div className="facility-session-copy"><strong>{item.customerDisplayName ?? '匿名顾客'}{item.plannedServiceItemName ? ` · ${item.plannedServiceItemName}` : ''}</strong>{item.expectedDurationMinutes && <span>预计 {item.expectedDurationMinutes} 分钟</span>}{item.note && <span>{item.note}</span>}<Typography.Text type="secondary">追溯编号 {item.visitNo}</Typography.Text>{canCheckout && <Typography.Text className="facility-enter-hint">点击卡片进入收银操作台</Typography.Text>}</div><Space wrap onClick={(event) => event.stopPropagation()}>{item.status === 'IN_USE' && <Button size="small" icon={<PauseCircleOutlined />} onClick={() => operation(item, 'pause')}>暂停</Button>}{item.status === 'PAUSED' && <Button size="small" icon={<PlayCircleOutlined />} onClick={() => operation(item, 'resume')}>继续</Button>}<Button size="small" icon={<SwapOutlined />} onClick={() => setSwitchTarget(item)}>换设施</Button><Button size="small" danger onClick={() => operation(item, 'end')}>结束服务</Button></Space></> : item.status === 'CLEANING_REQUIRED' ? <Button type="primary" onClick={(event) => { event.stopPropagation(); completeCleaning(item) }}>完成清洁</Button> : <Typography.Text type="secondary">{item.status === 'AVAILABLE' ? canCheckout ? '点击后直接进入收银操作台' : '点击后填写接待信息并开始使用' : '当前不可接待'}</Typography.Text>}
      </Card>})}</div></section>)}
    <Modal title={`开始使用 · ${startTarget?.displayName ?? ''}`} open={Boolean(startTarget)} onCancel={() => { setStartTarget(undefined); startForm.resetFields() }} onOk={() => startForm.submit()} confirmLoading={mutate.isPending} okText="确认开始" cancelText="取消" destroyOnHidden><Alert type="info" showIcon title="顾客和预计服务均可不填；它们只帮助识别接待，不生成收费。开始时间由服务器记录。" className="modal-alert" /><Form form={startForm} layout="vertical" onFinish={start}><Form.Item name="customerId" label="顾客/会员（可选）"><Select allowClear showSearch optionFilterProp="label" loading={customers.isLoading} placeholder="不选择则显示为匿名顾客" options={customers.data?.filter((item) => item.status === 'Active').map((item) => ({ value: item.id, label: `${item.displayName} · ${item.maskedMobile}` }))} /></Form.Item><Form.Item name="plannedServiceItemId" label="预计服务项目（可选）"><Select allowClear showSearch optionFilterProp="label" loading={serviceItems.isLoading} placeholder="仅作为待录单提示，不会自动收费" options={serviceItems.data?.filter((item) => item.status === 'ENABLED').map((item) => ({ value: item.id, label: `${item.name} · 标准时长 ${item.standardDurationMinutes} 分钟` }))} onChange={(value?: string) => { if (!value || startForm.getFieldValue('expectedDurationMinutes')) return; const item = serviceItems.data?.find((entry) => entry.id === value); if (item?.standardDurationMinutes) startForm.setFieldValue('expectedDurationMinutes', item.standardDurationMinutes) }} /></Form.Item><Form.Item name="expectedDurationMinutes" label="预计时长（分钟，可选）" rules={[{ type: 'number', min: 1, max: 1440 }]}><InputNumber min={1} max={1440} precision={0} className="full-width" /></Form.Item><Form.Item name="note" label="接待备注（可选）" rules={[{ max: 500 }]}><Input.TextArea rows={3} maxLength={500} showCount placeholder="不要填写不必要的敏感信息" /></Form.Item></Form></Modal>
