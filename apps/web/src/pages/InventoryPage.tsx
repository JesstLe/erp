...
import type { InventoryBalance, InventoryDocument, InventoryMovement } from '../api/types'
...
export function InventoryPage() {
  ...
  const movements = useQuery({ queryKey: ['inventory-movements', storeId], enabled: Boolean(storeId), queryFn: () => apiRequest<InventoryMovement[]>(`/api/v1/inventory/movements?storeId=${storeId}`) })
  const documents = useQuery({ queryKey: ['inventory-documents', storeId], enabled: Boolean(storeId), queryFn: () => apiRequest<InventoryDocument[]>(`/api/v1/inventory/documents?storeId=${storeId}`) })
  ...
  const onOpen = () => { const first = tracked[0]; form.setFieldsValue({ documentType: 'Receipt', reason: '', lines: first ? [{ productItemId: first.productItemId, quantity: 1 }] : [] }); setOpen(true) }
  ...
  return <div className="page-stack">
    <div className="page-heading"><div><Typography.Title level={2}>商品库存</Typography.Title><Typography.Paragraph>确认消费单只预占；完成收款才销售出库。设施计时与库存、收费均保持独立。</Typography.Paragraph></div>{owner && <Button type="primary" icon={<PlusOutlined />} disabled={!tracked.length} onClick={onOpen}>库存单据过账</Button>}</div>
    <Alert type="warning" showIcon title="库存流水过账后不可编辑或删除。产品退货只回库，不会自动发起资金退款；资金退款仍走原支付审批流程。" />
    ...
    <Card variant="borderless" title="最近库存流水" extra={<Typography.Text type="secondary">共 {movements.data?.length ?? 0} 条</Typography.Text>}><Table<InventoryMovement> rowKey="id" columns={movementColumns} dataSource={movements.data} loading={movements.isLoading} pagination={{ pageSize: 10 }} locale={{ emptyText: <Empty description="尚无库存流水" /> }} /></Card>
    <Card variant="borderless" title="最近过账单据"><Table<InventoryDocument> rowKey="id" size="small" dataSource={documents.data} loading={documents.isLoading} pagination={{ pageSize: 5 }} columns={[{ title: '单号', dataIndex: 'documentNo' }, { title: '类型', dataIndex: 'documentType', render: (value: string) => documentTypes.find((item) => item.value === value)?.label ?? value }, { title: '原因', dataIndex: 'reason' }, { title: '明细', dataIndex: 'lines', render: (lines: InventoryDocument['lines']) => lines.map((line) => `${line.productName} × ${line.quantity}${line.unitName}`).join('、') }, { title: '过账时间', dataIndex: 'postedAtUtc', render: (value: string) => new Date(value).toLocaleString('zh-CN') }]} /></Card>
    <Modal title="库存单据过账" width={760} open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} okText="确认过账" confirmLoading={post.isPending} destroyOnHidden><Alert type="warning" showIcon title="这是不可逆的事实过账。录错时请新增反向调整单，不得修改历史流水。" className="modal-alert" /><Form<DocumentValues> form={form} layout="vertical" onFinish={(values) => post.mutate(values)}><Form.Item name="documentType" label="单据类型" rules={[{ required: true }]}><Select options={documentTypes} /></Form.Item><Form.List name="lines" rules={[{ validator: async (_, lines) => { if (!lines?.length) throw new Error('至少添加一个产品') } }]}>{(fields, { add, remove }, { errors }) => <><Space orientation="vertical" className="full-width">{fields.map((field) => <Card key={field.key} size="small"><div className="payment-line-fields"><Form.Item name={[field.name, 'productItemId']} label="产品" rules={[{ required: true }]}><Select options={tracked.map((item) => ({ value: item.productItemId, label: `${item.productName} · 当前 ${item.onHandQuantity}${item.unitName}` }))} /></Form.Item><Form.Item name={[field.name, 'quantity']} label="数量" rules={[{ required: true }, { type: 'number', min: 1, max: 1000000000 }]}><InputNumber min={1} max={1000000000} precision={0} /></Form.Item></div>{fields.length > 1 && <Button danger onClick={() => remove(field.name)}>移除此行</Button>}</Card>)}</Space><Space><Button icon={<PlusOutlined />} onClick={() => add({ quantity: 1 })}>添加产品</Button><Form.ErrorList errors={errors} /></Space></>}</Form.List><Form.Item name="reason" label="变动原因" rules={[{ required: true, whitespace: true }, { max: 500 }]}><Input.TextArea rows={3} maxLength={500} showCount /></Form.Item></Form></Modal>
  </div>
}
