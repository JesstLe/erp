...
interface ItemForm { name: string; standardDurationMinutes: number; status: string; commissionMode: string; commissionRatePercent?: number; commissionFixedYuan?: number }
...
export function ServiceItemsPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: async (item) => { message.success(editing ? '服务项目已更新' : `服务项目已创建，系统编码 ${item.code}`); setOpen(false); setEditing(undefined); form.resetFields(); await refresh() },
  ...
  const showEdit = (item: ServiceItem) => { setEditing(item); form.setFieldsValue({ name: item.name, standardDurationMinutes: item.standardDurationMinutes, status: item.status, commissionMode: item.commissionMode ?? 'None', commissionRatePercent: item.commissionRateBasisPoints === undefined ? undefined : item.commissionRateBasisPoints / 100, commissionFixedYuan: item.commissionFixedMinor === undefined ? undefined : item.commissionFixedMinor / 100 }); setOpen(true) }
  const reset = () => { setQueryText(''); setStatus(undefined) }

  return <div className="page-stack">
    <div className="page-heading"><div><Typography.Title level={2}>服务项目</Typography.Title><Typography.Paragraph>支持查询、修改、停用与恢复；项目编码由系统按品牌自动升序生成且永久不变，最终收费由价格版本决定。</Typography.Paragraph></div>{canManage && <Button type="primary" icon={<PlusOutlined />} onClick={showCreate}>新建项目</Button>}</div>
