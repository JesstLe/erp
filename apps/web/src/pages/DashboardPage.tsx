...
export function DashboardPage() {
  ...
  const canCheckout = can(Permission.CashierCheckout)
  ...
  const report = useQuery({ queryKey: ['operations-report-today', storeId, date], enabled: Boolean(storeId && canViewReports), queryFn: () => apiRequest<OperationsReport>(`/api/v1/reports/operations?storeId=${storeId}&fromDate=${date}&toDate=${date}`) })
  const board = useQuery({ queryKey: ['facility-board', storeId], enabled: Boolean(storeId && canOperateFacilities), queryFn: () => apiRequest<FacilityBoard>(`/api/v1/facilities/board?storeId=${storeId}`) })
  ...
  return <div className="page-stack">
    ...
    {canViewReports ? <Row gutter={[16, 16]}>
      ...
      <Col xs={24} md={12} xl={6}><Card className="metric-card"><Statistic title="使用中设施" value={canOperateFacilities ? inUse : 0} suffix="个" prefix={<ClockCircleOutlined />} /><span>{canOperateFacilities ? '包含暂停中的设施' : '当前岗位不展示设施状态'}</span></Card></Col>
    ...
    </Row> : <Card><Typography.Title level={4}>岗位工作入口</Typography.Title><Typography.Paragraph type="secondary">经营金额、全店报表和审计数据仅向被授权岗位展示；你仍可使用下方已授权业务入口。</Typography.Paragraph></Card>}
    <Row gutter={[16, 16]}><Col xs={24} xl={15}><Card title="门店服务闭环" extra={<Tag color="green">已贯通</Tag>} className="flow-card"><div className="flow-steps">{['选择设施', '开始计时', '结束服务', '录入项目', '支付分摊', '交班审计'].map((label, index) => <div key={label}><span>{String(index + 1).padStart(2, '0')}</span><strong>{label}</strong></div>)}</div></Card></Col><Col xs={24} xl={9}><Card title="快速开始" className="quick-card"><Space orientation="vertical" size={12} className="full-width">{canOperateFacilities && <Button block size="large" onClick={() => navigate('/facilities')}>开始设施接待 <ArrowRightOutlined /></Button>}{canCheckout && <Button block size="large" onClick={() => navigate('/cashier')}>服务录单与收银 <ArrowRightOutlined /></Button>}{canViewReports && <Button block size="large" onClick={() => navigate('/reports')}>查看经营报表 <ArrowRightOutlined /></Button>}<Button block size="large" onClick={() => navigate('/catalog/items')}>查看服务项目 <ArrowRightOutlined /></Button></Space></Card></Col></Row>
  </div>
}
